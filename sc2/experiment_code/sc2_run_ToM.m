function [T, score] = sc2_run_ToM(screen,keyNames,T)
...
for trial = 1:numTrials,
    ...
    % Story Duration
    while GetSecs-t0 <= T.startTime(trial)+T.storyDur(trial);
        checkTR(screen);
    end;
    ...
    % Duration of delay (between story and question)
    ...
    while GetSecs-t0 <= T.startTime(trial)+T.questDur(trial);
        checkTR(screen)
        % Check the keyboard.
