function [T, score] = sc2_run_spatialMap(screen,keyNames,T)
...
if (nargin< 3 || isempty(T)) && isempty(keyNames{5}),
    DrawFormattedText(screen.window, sprintf('Spatial Working Memory Task! \n\n Use BOTH hands \n\n "%s" = 1 \n\n "%s" = 2 \n\n "%s" = 3 \n\n "%s" = 4',keyNames{1},keyNames{2},keyNames{3},keyNames{4}),...
...
% Number of actions
action={'makeSquares','drawNumbers','drawBorders','removeTargNum','drawTargBorder'}; 
...
for trial = 1:numTrials,
    ...
    makeSMDisplay(screen,A,'action',action{1})
    
    % Draw numbers to screen 
    makeSMDisplay(screen,A,'action',action{2})
    
    % Draw green border around respAlts
    makeSMDisplay(screen,A,'action',action{3})
    ...
    % length of delay 
    while GetSecs-t0 <= T.startTime(trial)+A.delayDur;
        checkTR(screen);
    end;
    ...
    % Remove target number from screen
     makeSMDisplay(screen,A,'action',action{4})
    
    % Draw target border to screen
    makeSMDisplay(screen,A,'action',action{5})
