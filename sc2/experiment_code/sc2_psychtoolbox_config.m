function [screen,keyNames] = sc2_psychtoolbox_config
% sc2
...
elseif screen.isScan == 1,
    screen.KbCheckList = [KbName('ESCAPE'), KbName('SPACE'),KbName('1'), KbName('2'), KbName('3'), KbName('4'), KbName('t'), KbName('5'), KbName('b'), KbName('y'), KbName('g'), KbName('r')];
    RestrictKeysForKbCheck(screen.KbCheckList);
    screen.escapeKey = KbName('ESCAPE');
    screen.spaceKey = KbName('SPACE'); 
    screen.pulseKey =  KbName('t');
    screen.one = KbName('1'); % b
    screen.two = KbName('2'); % y
    screen.three = KbName('3'); % g
    screen.four = KbName('4'); % r
    screen.pulseFive = KbName('5'); 
end
