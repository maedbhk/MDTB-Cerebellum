function [T, score] = sc2_run_landscapeMovie(screen,keyNames,T)
...
if (nargin < 3 || isempty(T)) && isempty(keyNames{5}),
    DrawFormattedText(screen.window, 'Watch the following movie', ...
        'center', 'center', screen.black);
    Screen('Flip', screen.window);
    return;
elseif (nargin < 3 || isempty(T)) && ~isempty(keyNames{5}),
    DrawFormattedText(screen.window, sprintf('Watch the following movie \n\n %s',keyNames{5}),...
        'center', 'center', screen.black);
    Screen('Flip', screen.window);
    return;
end;
