function sc1_runExperiment(subjName,runFile,feedback,timing,eyeTracking,runNum)
global TRreal timeOfTR;
%% super_cerebellum project - study 1
...
% subjName    = 's01'; any format can be adopted
% runFile     = 'sc1_run1.txt'; This (example) file provides all of the information
...
cd(rootDir)
resDir = [rootDir, '/data/sc1/behavioural', sprintf('/%s', subjName)]; dircheck(resDir);
eyeTrackDir = [rootDir, '/data/sc1/eyeTracking',sprintf('/%s',subjName)]; dircheck(eyeTrackDir);
runDir=[rootDir, '/data/sc1/run_files']; dircheck(runDir);
taskDir=[rootDir, '/data/sc1/task_files']; dircheck(taskDir);
...
runFileName = fullfile(resDir,sprintf('sc1_%s.dat',subjName));
Dres        = dload(runFileName);

% configure eyetracker
if eyeTracking==1,
    [el]=sc1_config_eyeTracking;
end
...
% open screen
[screen,keyNames] = sc1_psychtoolbox_config; % set-up psychtoolbox parameters (screen,keyboard etc)
...
for b=1:numTasks,
    ...
    taskFileName = fullfile(resDir,sprintf('sc1_%s_%s.dat',subjName,D.taskName{b}));
    ...
    % start eyetracking
    if eyeTracking==1,
        if strcmp(T{b}.taskName(1),'motorImagery') || strcmp(T{b}.taskName(1),'motorSequence') || strcmp(T{b}.taskName(1),'nBack')|| strcmp(T{b}.taskName(1),'nBackPic'),
        ...
        [edfFile] = sc1_start_eyeTracking(screen,el,edfFile,T{b});
    end
    
    % display instructions
    if strfind(D.taskName{b},'spatialNavigation')==1, % Special case - spatial navigation
        P{1} = T{b};
...
if (feedback==0) && exist('scores','var'),
    ...
    feval('sc1_endExperiment', screen) % End experiment after last trial
else
