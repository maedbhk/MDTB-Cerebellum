function [T,score] = sc1_run_motorSequence(screen,keyNames,T)
...
for trial = 1:numTrials,
    ...
    for i=flip(1:numResp)
        DrawFormattedText(screen.window, num2str(mov(trial,i)), screen.xCenter-charXpos(i), screen.yCenter, screen.black);
    end;
    Screen('Flip', screen.window);
        ...
        if (~keyDown && isPressed && respMade<numResp && (keyCode(screen.one) || keyCode(screen.two) || keyCode(screen.three) || keyCode(screen.four)))
            ...
            for i= flip(1:numResp)
                ...
                DrawFormattedText(screen.window, num2str(mov(trial,i)), ...
                    screen.xCenter-charXpos(i), screen.yCenter, ...
                    colour);
            end;
            Screen('Flip', screen.window);
        end;
